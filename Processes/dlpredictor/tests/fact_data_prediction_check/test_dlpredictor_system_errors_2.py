...
number_of_uckeys_to_read = 10000 # 10000 is the largest we can receive from Elasticsearch in a single batch.
...
factdata_table = 'dlpm_05182021_1500_tmp_area_map'
...
# Gets the predicted counts from Elasticsearch.  Will get predictions for the given
# uckeys if passed.  Otherwise, will get predictions for number_of_uckeys_to_read number
# of uckeys.
def get_predictions(uckeys = None):
    ...
    for ucdoc in hits:
        ...
        for day, hours in predictions.items():
            ...
            for hour_doc in hours['hours']:
                # hour += 1
                # predicted_counts[(uckey, day, hour, '0')] = hour_doc['h0']
                # predicted_counts[(uckey, day, hour, '1')] = hour_doc['h1']
                # predicted_counts[(uckey, day, hour, '2')] = hour_doc['h2']
                # predicted_counts[(uckey, day, hour, '3')] = hour_doc['h3']
                h0 += hour_doc['h0']
                h1 += hour_doc['h1']
                h2 += hour_doc['h2']
                h3 += hour_doc['h3']
            ...
            predicted_counts[(uckey, day, '0')] = h0
            predicted_counts[(uckey, day, '1')] = h1
            predicted_counts[(uckey, day, '2')] = h2
            predicted_counts[(uckey, day, '3')] = h3
            predicted_counts[(uckey, day)] = h0 + h1 + h2 + h3
            # print('daily: {}  {}  {}  {} : {}'.format(h0, h1, h2, h3, h0+h1+h2+h3))
    if uckeys:
        print('found: {}  requested: {}'.format(len(found_uckeys), len(uckeys)))
    else:
        print('found: {}  requested: {}'.format(len(found_uckeys), number_of_uckeys_to_read))
    return predicted_counts, found_uckeys, predicted_days
...
def generate_command(table, uckeys, bucket_id):
    ...
    return 'select * from {} where bucket_id = {} AND uckey in ({})'.format(
        table, bucket_id, ','.join(formatted_uckeys))
...
if __name__ == '__main__':
    ...
    print('0: Load the predictions')
    predicted_counts, uckeys, predicted_days = get_predictions()
    ...
    for bucket_id in bucket_uckeys.keys():
        ...
        for uckey in uckeys:
            ...
            for day in sorted(all_days):
                ...
                # Add the day.
                observed_counts[(uckey, day)] = count
        ...
        with open(output_filepath, 'w') as f:
            ...
            f.write('uckey,slot_type,slot_id,network,gender,age,price_type,region,ipl,num_days,actual_total,predicted_total')
            ...
            for uckey, found, agg_error, count, pcount in zip(all_uckeys, all_found, all_agg_errors, all_counts, all_pcounts):
                f.write('"{}",{},{},{},{}'.format(uckey, uckey, found, count, pcount))

                # Print the actual data.
                for day in all_days_sorted:
                    key = (uckey, day)
                    if key in observed_counts:
                        f.write(',{}'.format(observed_counts[key]))
                    else:
                        f.write(',0')
