...
uckey_file = 'dense_uckeys.txt'
uckey_path = os.path.join(os.path.join(
        os.path.dirname(os.path.realpath(__file__)), 'data_source'), uckey_file)
...
output_filepath = 'dense_dlpredictor.csv'

# The number of uckeys to load per batch.
step_size = 500  # 500 is the approximate maximum before the queries throw errors.
...
def generate_command(table, uckeys):
    ...
    return 'select * from {} where ({})'.format(table, ' OR '.join(conditions))
...
def get_observed(hive_context, uckeys):
    ...
    for row in df.collect():
        ...
        for count in ts:
            ...
            day = day + day_delta
    return observed_counts, observed_days
...
if __name__ == '__main__':
    ...
    with open(output_filepath, 'w') as f:
        for i in segment(len(dense_uckeys), step_size):
            ...
            predicted_counts, found_uckeys, predicted_days = get_predictions(es, uckeys)
            ...
            observed_counts, observed_days = get_observed(hive_context, uckeys)
