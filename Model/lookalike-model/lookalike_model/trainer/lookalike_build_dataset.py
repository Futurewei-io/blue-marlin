...
def __data_parser(serialized_example):
    ...
    features = tf.parse_single_example(serialized_example,
                                       features={'interval_starting_time': tf.FixedLenSequenceFeature([], tf.string, allow_missing=True),
                                                 ...
                                                 'aid_index': tf.FixedLenFeature([], tf.int64),
                                                 ...
                                                 'gender_index': tf.FixedLenFeature([], tf.int64),
                                                 'age': tf.FixedLenFeature([], tf.int64, 0),
                                                 'aid': tf.FixedLenFeature([], tf.string),


                                                 })
    did_str = tf.cast(features['aid'], tf.string)
    time_interval = tf.cast(features['interval_starting_time'], tf.string)
    keyword = tf.cast(features['kwi'], tf.string)
    ucdoc = tf.cast(features['aid_index'], tf.int64)
    ...
    gender = tf.cast(features['gender_index'], tf.int64)
...
def run(cfg):
            ...
            for m in range(len(click_counts_test)):
                for n in range(len(click_counts_test[m])):
                    if (click_counts_test[m][n] != 0):
                        pos = (ucdoc, flatten(keyword_int[m+1:m+1+length]), (keyword_int[m][n], keyword_int[m][n]))
                        pos_sample.append(1)
                        if len(pos[1]) >= 1:
                            test_set.append(pos)
                            label.append(1)
                    elif (m % 5 == 0 and n % 2 == 0):
                        neg = (ucdoc, flatten(keyword_int[m+1:m+1+length]), (keyword_int[m][n], keyword_int[m][n]))
                        if len(neg[1]) >= 1:
                            test_set.append(neg)
                            label.append(0)
    ...
    cate_list = np.array([x for x in range(15)])
    # cate_list = np.array([i for i in keyword_set])
    user_count, item_count, cate_count = len(set(ucdoc_lst)), 15, 15
    print(counter)
    print(s)
    # mapping = pd.DataFrame(mapping)
    # mapping.to_csv("mapping_pipeline.csv")
    with open('label_lookalike_i.pkl', 'wb') as f:
        pickle.dump(label, f, pickle.HIGHEST_PROTOCOL)
        pickle.dump(user_att, f, pickle.HIGHEST_PROTOCOL)

    with open('ad_dataset_lookalike_i.pkl', 'wb') as f:
