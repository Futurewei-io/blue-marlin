...
from lookalike_model.trainer.dice import dice
from lookalike_model.trainer.rnn import dynamic_rnn
...
class Model(object):
    ...
    def __init__(self, user_count, item_count, cate_count, cate_list, predict_batch_size, predict_ads_num, u, i, y,
                 hist_i, sl):
        self.u = u
        self.i = i
        ...
        self.lr = tf.constant(0.001, shape=[], dtype=tf.float64)
        '''
        self.u: user
        self.i: positive sample (clicked)
        self.j: negative sample (not clicked)
        self.y: label (positive or negative)
        self.hist_i: list of historical keywords (behavior)
        self.sl: length of historical keywords
        self.lr: learning rate
        '''

        hidden_units = 128

        # user embedding commented as the embedding vector gives memory error
        ...
        if USE_RNN:
            rnn_outputs, _ = dynamic_rnn(GRUCell(hidden_units), inputs=h_emb, sequence_length=self.sl, dtype=tf.float32,
                                         scope='gru1')
    ...
    def assign_placeholders(self, ucdoc, click_count, label, hist_keyword_list, sl, lr):
        self.u = ucdoc
        self.i = click_count
        self.y = label
        self.hist_i = hist_keyword_list
        self.sl = tf.constant(4, shape=[], dtype=tf.int32)
    ...
    def train_new(self, sess, ucdoc, click_count, label, hist_keyword_list, sl, lr):
