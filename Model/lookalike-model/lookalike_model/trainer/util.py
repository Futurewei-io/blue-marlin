...
def add_index(df, column_name, column_index_name, drop_column=True):
    df_distinct = df.select(column_name).distinct().orderBy(column_name)
    df_distinct_indexed = index_distinct_df(df_distinct, column_name, column_index_name)
    df = df.join(df_distinct_indexed, on=[column_name], how="inner")
    if drop_column:
