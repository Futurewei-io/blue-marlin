...
RNN_COMPATIBLE_CELL = cudnn_rnn.CudnnCompatibleGRUCell
    ...
    if RNN == tf.contrib.cudnn_rnn.CudnnLSTM:
        ...
        if USE_COMPATIBLE:
            with tf.variable_scope('cudnn_gru'):
                single_cell = lambda: cudnn_rnn.CudnnCompatibleGRUCell(num_units=hparams.rnn_depth, reuse=None,
                                                                       kernel_initializer=tf.initializers.random_uniform(
                                                                           minval=-0.05, maxval=0.05, seed=seed + 1 if seed else None
                                                                       ))
                cell = tf.nn.rnn_cell.MultiRNNCell([single_cell() for _ in range(hparams.encoder_rnn_layers)])
                rnn_out_wrong_order, (rnn_state_wrong_order,) = tf.nn.dynamic_rnn(cell, time_inputs, dtype=tf.float32)
...
class Model:
    ...
    def __init__(self, inp: InputPipe, hparams, is_train, seed, graph_prefix=None, asgd_decay=None, loss_mask=None):
        ...
        if USE_ATTENTION:
            ...
            decoder_targets, decoder_outputs = self.decoder(encoder_state, None, inp.time_y, inp.norm_x[:, -1])
