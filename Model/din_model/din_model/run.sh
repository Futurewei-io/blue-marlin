...
then
    ...
    spark-submit --master yarn --num-executors 20 --executor-cores 5 --executor-memory 8G --driver-memory 8G --conf spark.driver.maxResultSize=5g pipeline/main_clean.py config.yml
fi

# main_logs: generating union logs and cleaning the logs.
if false
then
    spark-submit --master yarn --num-executors 20 --executor-cores 5 pipeline/main_logs.py config.yml 
fi

# Optional step: main_logs_with_regions: inject region ids to the logs table which don't have any geo or region information.
if false
then
    spark-submit --master yarn --num-executors 20 --executor-cores 5 pipeline/main_logs_with_regions.py config.yml 
fi

# main_trainready: generating the trainready data by grouping the data.
if false
then
    spark-submit --master yarn --num-executors 20 --executor-cores 5 pipeline/main_trainready.py config.yml 
fi
