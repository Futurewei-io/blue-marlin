...
def __data_parser(serialized_example):
    ...
    features = tf.parse_single_example(serialized_example,
                                       features={'interval_starting_time': tf.FixedLenSequenceFeature([], tf.int64, allow_missing=True),
                                                 ...
                                                 'uckey_index': tf.FixedLenFeature([], tf.int64),
                                                 ...
                                                 'age_index': tf.FixedLenFeature([], tf.int64),
    ...
    ucdoc = tf.cast(features['uckey_index'], tf.int64)
    ...
    age = tf.cast(features['age_index'], tf.int64)

    return time_interval, ucdoc, click_counts, show_counts, media_category, net_type_index, gender, age, keyword
...
def run(cfg):
    ...
    tfrecord_location = cfg['pipeline']['tfrecord_location']
    names = []
    ...
    for i in range(ucdoc_num):
        x = sess.run(next_el)
        log = list(x[0:9] )
        time_interval, ucdoc, click_counts, show_counts, media_category, net_type_index, gender, age, keyword = log[0], log[1], log[2], log[3], log[4], log[5], log[6], log[7], log[8]
        ...
        dense_att = [ucdoc, media_category, net_type_index, gender, age]
